...
func TestRAND(t *testing.T) {
	...
	*/
	...
	//SQN_str := "0000000ad010"
	SQN_str := "000000000000"
	SQN, _ := hex.DecodeString(SQN_str)

	milenage.GenerateOPC(K, OP, OPC)

	fmt.Printf("K=%x\nSQN=%x\nOP=%x\nOPC=%x\n", K, SQN, OP, OPC)
	//(0x):1912d20ce43eedd767facf57e8646918
	//     1912d20ce43eedd767facf57e8646918
	RAND := make([]byte, 16)
	// rand.Read(RAND)
	// RAND, _ = hex.DecodeString("0100000000000000f92ea516d87d7de0")
	...
	// AUTN   e7 ac b5 2d 05 ed 80 00 9b 24 29 ff 86 a9 93 32
	expAUTN, _ := hex.DecodeString("5823fa3848398000281029eee80503e6")
	if !reflect.DeepEqual(AUTN, expAUTN) {
		t.Errorf("AUTN[0x%x] \t expected[0x%x]\n", AUTN, expAUTN)
	}
	//b3e1413492028000ec47a841a698194f
	//b3e1413492028000ec47a841a698194f
